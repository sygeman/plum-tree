...
export const ImageManager = ({
  ...
  function fileSelected(ev) {
    ...
    // get S3 signed URL
    axios
      .get(`/api/upload/url?type=${type}&dir=${dir}`)
      .then((response) => {
        ...
        // upload to S3
        return axios.put(response.data.uploadURL, file, options);
      })
      .then(() => {
      ...
      .catch((error) => {
        toast.error(
          get(
            ...
            "Unknown error occurred uploading your file"
          ),
          { autoClose: false }
        );
      });
  ...
  function cropImage(e) {
    ...
    axios
      .put("/api/upload/crop", { cropData, image })
      .then((response) => {
      ...
      .catch((error) => {
        toast.error(
          get(
            error,
            "response.data.errors[0].detail",
            "Unknown error occurred while cropping image"
          ),
          { autoClose: false }
        );
      });
  ...
  if (cropping) {
    return <Loading message="Cropping image..." />;
  }

  if (showCropper) {
    return (
      <div>
        <ReactCrop aspect={aspect} crop={crop} onChange={onCropChange}>
          <img onLoad={onImageLoaded} src={cropImageUri} />
        </ReactCrop>
        <div className="text-center">
          <button className="btn btn-default" onClick={cancelCrop}>
            Cancel Crop
          </button>
  ...
  return (
    <div className="text-center">
      {imagePreview}
      <button className="btn btn-link m-2" onClick={selectImage}>
        Upload Image
      </button>
      <button
